...
void Client::move_figure(int id, std::array<int,2> pos_to){
    rapidjson::Document doc;

    auto& allocator = doc.GetAllocator();

    Request move_figure_msg;

    move_figure_msg.m_type = RequestType::MOVE_FIGURE;

    rapidjson::Value value(rapidjson::kObjectType);
    rapidjson::Value stringValue;
    stringValue.SetString(id.data(), allocator);
    value.AddMember("id",  stringValue, allocator);
    
    
    value.AddMember("pos-to", rapidjson::Value(rapidjson::kObjectType), allocator);
    stringValue.SetString(pos_to[0].data(), allocator);
    value["pos-to"].AddMember("row", stringValue, allocator);
    stringValue.SetString(pos_to[1].data(), allocator);
    value["pos-to"].AddMember("column", stringValue, allocator);

    move_figure_msg.m_jsonData = valueToString(value);
...
void Client::handle_move_figure(const boost::system::error_code& err){
      std::cout<<"handle move figure"<<std::endl;
        if (err)
        {
            std::cout << "Error: " << err << "\n";
        }
